...
#define SOUND_BOOST_CONTROL_VERSION             1
/* Headset Audio Configs */ 
			 ...
			 const char *buf, size_t count)
{
	...
	if (sound_boost_status == 0)
	{
		...
		snd_soc_write(sound_boost_codec, REG_ANAGAIN3, REG_ANAGAIN3_HSRGAIN);
		snd_soc_write(sound_boost_codec, REG_ANAGAIN3, REG_ANAGAIN3_HSLGAIN);
	}
	if (sound_boost_status == 1)
	{
		printk(KERN_DEBUG "[ab8500-Sound-Control] Sound boost enabled\n");
		snd_soc_write(sound_boost_codec, REG_ANAGAIN3, REG_ANAGAIN3_HSRGAIN_BOOST);
		snd_soc_write(sound_boost_codec, REG_ANAGAIN3, REG_ANAGAIN3_HSLGAIN_BOOST);
	}
	else
	{
